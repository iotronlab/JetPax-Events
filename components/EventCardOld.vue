  ...
  <v-card class="mb-12 mx-4 pb-2 pt-1" style="border-radius: 10px">
    ...
    >
      <v-list-item class="my-2">
        <v-btn text tile class="white py-8" style="border-radius: 10px">
          <div>
            <v-card-title
              class="ma-0 pa-0 text--lighten black--text mt-1 ml-2"
              >{{ StartDate(event.startOn).split(' ')[0] }}</v-card-title
            >
            <v-card-text
              class="ma-0 pa-0 text--lighten black--text text-body-2"
              >{{ StartDate(event.startOn).split(' ')[1] }}</v-card-text
            >
          </div>
        </v-btn>
        <v-spacer></v-spacer>
        <v-btn icon class="red"
          ><v-icon class="">{{ icons.heart }}</v-icon></v-btn
        >
      </v-list-item>
    </v-img>
    ...
    <v-card-text class="mb-0 pb-0 mt-4">
      <v-row align="center" class="mx-0">
        <div class="grey--text">
          <v-icon small class="mr-1">{{ icons.calendar }}</v-icon>
          {{ FullDate(event.startOn, event.endOn) }}
        </div>
      </v-row>

      <div class="mt-6 text-body-2">
        <v-icon small class="mr-1">{{ icons.marker }}</v-icon>
        {{ event.location }}
      </div>
    </v-card-text>
    ...
    <v-card-actions class="mx-2 mt-2">
      <v-btn color="deep-purple lighten-2" rounded text> Bookings open </v-btn
      ><v-spacer></v-spacer>
